...
<template>
  <v-card outline block class="ma-3" hover>
    ...
    <v-data-table :hidden=isHidden class="ma-6" :search="search" :headers="headers" :items="this.$store.state.orgs.organizations">
      ...
      <template v-slot:top>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="blue-grey darken-1 white--text" dark class="mb-2" v-on="on">New Organization</v-btn>
          </template>
          <v-card>
            <v-card-title>
